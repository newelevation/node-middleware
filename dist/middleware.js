  ...
  const pipeline = (insertions = []) => {
    return async (input, output) => {
      ...
      const next = async (input2, output2) => {
        ...
        if (current) {
          return await invoke(current, next, input2, output2);
        }
        return output2;
      };
      ...
      return output;
      async function invoke(node, next2, input2, output2) {
        if ((0, import_lodash.isFunction)(node)) {
          return await node(next2)(input2, output2);
        } else {
          return await node[1](next2)(input2, output2);
        }
      }
    };
  };
